...
class Feed(object):
    ...
    _default_configured_attributes[_default_configured_attributes.index("from")] = (
    ...
    _configured_attribute_translations.update(
        dict(zip(_default_configured_attributes, _config.CONFIG["DEFAULT"].keys()))
    )
    ...
    @property
    def user_agent(self):
        return self._user_agent.replace("__VERSION__", __version__).replace(
            "__URL__", __url__
        )
    ...
    def __init__(self, name=None, url=None, to=None, config=None):
        ...
        self.reset()
        self.__setstate__(
            dict((attr, getattr(self, attr)) for attr in self._dynamic_attributes)
        )
    ...
    def save_to_config(self):
        ...
        for attr in self._configured_attributes:
            ...
            if value is not None:
                value = self._get_configured_option_value(attribute=attr, value=value)
                if (
                    attr in self._non_default_configured_attributes
    ...
    def load_from_config(self, config=None):
        ...
        if keys != expected:
            for key in expected:
                if (
                    key not in keys
                    and key not in self._non_default_configured_attributes
                ):
    ...
    def _fetch(self):
        ...
        f = _util.TimeLimitedFunction(
            "feed {}".format(self.name), timeout, _feedparser.parse
        )
        return f(
            self.url, self.etag, modified=self.modified, agent=self.user_agent, **kwargs
        )
    ...
    def _check_for_errors(self, parsed):
        ...
        if status in [301, 308]:
            _LOG.info(
                "redirect {} from {} to {}".format(self.name, self.url, parsed["url"])
            )
            self.url = parsed["url"]
            # TODO: `url` is not saved -- add config option to call feeds.save_config() in run command
        elif status == 304:
            _LOG.info(
                "skipping {}: feed was not modified since last update".format(
                    self.name, self.url
                )
            )
            return
        elif status == 410:
            _LOG.warning("deactivate {} because {} is gone".format(self.name, self.url))
            self.active = False
        ...
        if isinstance(exc, _socket.timeout):
            ...
            warned = True
        elif parsed.bozo and isinstance(exc, _feedparser.CharacterEncodingOverride):
            _LOG.warning("incorrectly declared encoding: {}: {}".format(exc, self))
            warned = True
        ...
        if not warned and status in [200, 302] and not parsed.entries and not version:
    ...
    def _process_entry(
        self, parsed, entry
    ) -> Optional[Tuple[str, Dict[str, Any], str, Message]]:
        ...
        if self.bonus_header:
            for header in self.bonus_header.splitlines():
                if ":" in header:
                    ...
                    _LOG.warning("malformed bonus-header: {}".format(self.bonus_header))
    ...
    def _get_entry_tags(self, entry):
        ...
        ...                   'label': None}]})
        """
        taglist = [tag["term"] for tag in entry.get("tags", []) if tag.get("term", "")]
        if taglist:
            return ",".join(taglist)
        ...
        if self.html_mail:
            ...
            for elink in getattr(entry, "links", []):
                if elink.get("rel", None) == "via":
                    ...
                    lines.append(
                        '<p>Via <a href="{}">{}</a></p>'.format(
                            _saxutils.escape(url), _saxutils.escape(title)
                        )
                    )
            lines.extend(
                ["</div>", "</div>", "</body>", "</html>", ""]  # /footer  # /entry
            )
    ...
    def _send(self, sender, message):
        ...
        if section not in self.config:
            section = "DEFAULT"
        _email.send(
            recipient=self.to, message=message, config=self.config, section=section
        )
    ...
    def _new_digest(self):
        ...
        digest["To"] = _formataddr(
            _parseaddr(self.to)
        )  # Encodes with utf-8 as necessary
